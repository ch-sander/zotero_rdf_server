...
import { registerRemoteSingleLoader }
    from "./remote-single-loader.js";
...
const ENTITY_TYPES_TTL = ENTITY_TYPES
    .map(iri => `<${iri}>`)
    .join("\n        ");
...
#################################################################
# Read-only metadata group
#################################################################

sshap:ReadOnlyMetadataGroup
    a sh:PropertyGroup ;
    rdfs:label "Metadata"@en, "Metadaten"@de ;
    sh:order 90 .

#################################################################
# Type-independent read-only metadata
#################################################################

sshap:GeneratedByReadOnlyProperty
    a sh:PropertyShape ;
    sh:path prov:wasGeneratedBy ;
    sh:name "Generated by"@en, "Erzeugt durch"@de ;
    sh:nodeKind sh:IRI ;
    sh:maxCount 1 ;
    dash:readonly true ;
    sh:group sshap:ReadOnlyMetadataGroup ;
    sh:order 91 .


sshap:DescriptionReadOnlyProperty
    a sh:PropertyShape ;
...
sshap:CreativeCommonsLicenseReadOnlyProperty
    a sh:PropertyShape ;
    sh:path cc:license ;
    sh:name "Creative Commons license"@en,
            "Creative-Commons-Lizenz"@de ;
    sh:nodeKind sh:IRI ;
    sh:maxCount 1 ;
    dash:readonly true ;
    sh:group sshap:ReadOnlyMetadataGroup ;
...
sshap:FundingReadOnlyProperty
    sh:deactivated true ;
    a sh:PropertyShape ;
    sh:path schema:funding ;
    sh:name "Funding project"@en, "Förderprojekt"@de ;
    sh:nodeKind sh:IRI ;
    sh:maxCount 1 ;
    dash:readonly true ;
    sh:group sshap:ReadOnlyMetadataGroup ;
...
sshap:MappingRdfsLabelProperty
a sh:PropertyShape ;
sh:path rdfs:label ;
sh:name "RDFS label"@en, "RDFS-Bezeichnung"@de ;
...
sshap:EntityZoteroTagsProperty
    a sh:PropertyShape ;
    sh:path zot:tags ;
    sh:name "Zotero tags"@en, "Zotero-Tags"@de ;
    sh:datatype xsd:string ;
    sh:group sshap:CommonEntityGroup ;
    sh:order 7 .
...
const includeCompleteRdfEl = $("includeCompleteRdf");
const listEl = $("list");
const qEl = $("q");
...
const exportShapesBtn = $("exportShapes");
...
const remoteMappingIriEl = $("remoteMappingIri");
const loadRemoteMappingBtn = $("loadRemoteMapping");
...
if (endpointFromUrl) {
    endpointEl.value = endpointFromUrl;
    remoteEndpoint = endpointFromUrl;
    updateSourceInfo();
}
...
function debugSubjectValues(
    ...
    const graph = DF.namedNode(graphIri);
...
async function previewSelectedNow_old() {
...
async function buildMappingQuads({ complete = false } = {}) {
    if (!selectedMapping) return [];

    const formQuads = parseSerializedToQuads(
        await formToTriG(mappingForm),
        "application/trig"
    );
    ...
    const originalQuads = collectSubjectClosureFromStore(
        dataset,
        G_MAPPINGS,
        selectedMapping
    );

    const managedPredicates = new Set(FORM_PROPS);

    const unmanagedQuads = originalQuads.filter(q =>
        !managedPredicates.has(q.predicate.value)
    );

    return deduplicateQuads([
        ...formQuads,
        ...unmanagedQuads
    ]);
}

async function buildEntityQuads({ complete = false } = {}) {
    if (!selectedTarget) return [];

    const formQuads = parseSerializedToQuads(
        await formToTriG(entityForm),
        "application/trig"
    );
...
async function buildMappingTriG() {
    const quads = await buildMappingQuads({
        complete: includeCompleteRdfEl.checked
    });
...
async function buildEntityTriG() {
    const quads = await buildEntityQuads({
        complete: includeCompleteRdfEl.checked
    });
...
async function buildSelectedTriG() {
    const complete = includeCompleteRdfEl.checked;

    const parts = [
        ...await buildMappingQuads({ complete }),
        ...await buildEntityQuads({ complete })
    ];

    if (!parts.length) return "";

    return writeTriG(
        deduplicateQuads(parts),
        PREVIEW_PREFIXES
    );
}

async function previewSelectedNow() {
    const trig = await buildSelectedTriG();
...
showMappingBtn.addEventListener("click", async () => {
    try {
        const trig = await buildMappingTriG();

        if (!trig) {
            setStatus("No mapping selected.");
            return;
        }
...
showEntityBtn.addEventListener("click", async () => {
    try {
        const trig = await buildEntityTriG();

        if (!trig) {
            setStatus("No entity selected.");
            return;
        }
...
exportSelectedBtn.addEventListener("click", async () => {
    try {
        ...
        downloadText("mapping+entity.trig", trig);
    } catch (error) {
        setStatus(
            error instanceof Error
                ? error.message
...
async function prettyTurtle(turtleText) {
    const parser = new window.N3.Parser({
        format: "text/turtle"
    });
...
exportShapesBtn.addEventListener("click", () => {
    try {
        if (!SHAPES_TTL?.trim()) {
            throw new Error("No SHACL shapes available.");
        }
        ...
        setStatus("SHACL shapes downloaded.");
        setTimeout(() => setStatus(""), 1200);
    } catch (error) {
        setStatus(
            error instanceof Error
...
async function selectEntity(entityIri) {
    selectedMapping = null;
    selectedTarget = entityIri;

    const entityTTL = await loadEntityTTL(entityIri);

    mappingForm.setAttribute("data-values", "");
    mappingForm.removeAttribute("data-values-subject");

    entityForm.setAttribute("data-values", entityTTL);
    entityForm.setAttribute("data-values-subject", entityIri);
...
registerRemoteSingleLoader({
    ...
    hasLocalSubject: (graphIri, subjectIri) => {
        const DF = window.N3.DataFactory;

        return dataset.countQuads(
            DF.namedNode(subjectIri),
            null,
            null,
            DF.namedNode(graphIri)
        ) > 0;
    },
    clearCachedMapping: iri => {
        savedTTL.mappings.delete(iri);
    },

    clearCachedEntity: iri => {
        savedTTL.entities.delete(iri);
    }
});