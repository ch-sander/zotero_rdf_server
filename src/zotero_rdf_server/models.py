...
class ZoteroLibrary:
    def __init__(self, config: dict, check: bool = True):
        ...
        if check:
            ...
            if not str(self.library_id).isdigit() and not self.library_type in ["knowledge base", "mapping", "dataset", "query", "update", "citation"]:
                passing = False
            ...
            if not self.library_type in ["groups", "user", "knowledge base", "mapping", "dataset", "query", "update", "citation"]:            
                passing = False
    ...
    def fetch_paginated(self, endpoint: str) -> list:
        ...
        def normalize_collections(value) -> list[str]:
            ...
            raise TypeError(
                "Parameter 'collection' must be None, String, CSV-String or List/Tuple/Set."
            )
